...
const QuizArea: React.FC<QuizAreaProps> = ({
  ...
  // 디버깅: 학습 모드일 때 topicContent 확인
  if (quizMode === 'learning') {
    console.log('학습 모드 - selectedTopic:', selectedTopic, 'topicContent:', topicContent ? '있음' : '없음');
  }
  
  // 학습 모드: 학습 내용만 보여주고 퀴즈는 숨김
  if (quizMode === 'learning') {
    // topicContent가 없는 경우 안내 메시지
    if (!topicContent) {
      return (
        <div className="max-w-4xl mx-auto animate-fade-in-up">
          ...
          <div className="p-8 bg-yellow-50 border-l-4 border-yellow-500 rounded-xl shadow-sm">
            <p className="text-yellow-800 text-lg">
              이 주제에 대한 학습 자료가 아직 준비되지 않았습니다.
            </p>
          </div>
        </div>
      );
        ...
        {hasPassedExam && (
          <div className="p-6 bg-emerald-50 border-l-4 border-emerald-400 rounded-xl mb-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-bold text-emerald-800">시험 통과!</h3>
            </div>
